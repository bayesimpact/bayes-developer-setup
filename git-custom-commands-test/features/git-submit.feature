Feature: git submit
  ...
  Scenario: Successful submission
    ...
    And I create a "success" git branch from default branch
    ...
    And I should be on "default_branch" git branch
    And the "success" git branch should not exist
    And the "success" git branch in "origin" should not exist
    And the default_branch git branch should be in sync with "default_branch" in "origin"
  ...
  Scenario: Branch to submit is late compared to origin/default_branch
    Given a dummy git repo in "origin"
    And I am in a "work" git repo cloned from "origin"
    And a file "other dev" is committed on default branch git branch in "origin" with:
      """
      Whatever
      """
    And I create a "success" git branch from default branch
  ...
  Scenario: Merge conflict on submission
    Given a dummy git repo in "origin"
    And I am in a "work" git repo cloned from "origin"
    And a file "conflict" is committed on default branch git branch in "origin" with:
      """
      Whatever
      """
    And I create a "success" git branch from default branch
