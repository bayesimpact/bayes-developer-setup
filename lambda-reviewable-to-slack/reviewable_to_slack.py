...
_REVIEWABLE_COMMENT_REGEX = re.compile(
    ...
    r'---\n\n(?P<inline_comments>(.|\n)*)$',
    re.MULTILINE)
_REVIEWABLE_INLINE_COMMENT_LINK_REGEX = re.compile(
    r'https://reviewable\.io[^)]+', re.MULTILINE)
_REVIEWABLE_ASSIGN_REGEX = re.compile(r'\+@([\w]+)\b', re.MULTILINE)
_REVIEWABLE_DEMO_REGEX = re.compile(
    r'.*(Demo ready for review|No demo to review)', re.MULTILINE)
_REVIEWABLE_HTML_EMOJI_REGEX = re.compile(
    r'<img class="emoji" title="([^"]+)"[^>]*>')
...
def _generate_slack_messages_for_new_comment(issue, new_comment):
    ...
    new_comment_assignees = _REVIEWABLE_ASSIGN_REGEX.findall(
        new_comment['body'])
    ...
    has_unaddressed_comments = not unaddressed_comment_count
    ...
    if new_comment_is_demo_ready:
        ...
        for assignee in assignees:
            add_slack_message(
                assignee, ReviewableEvent.ASSIGNED, CallToAction.REVIEW, issue_owner)
...
def _generate_slack_message(from_user, event, to_user, call_to_action, issue):
    ...
    repository_name = _GITHUB_REPO_NAME_REGEX.match(
        issue['repository_url']).group(1)
    reviewable_url = 'https://reviewable.io/reviews/{}/{}'.format(
        repository_name, issue['number'])
    ...
    call_to_action_string = _generate_call_to_action_slack_string(
        call_to_action, reviewable_url)
    slack_message = '_{}:_\n{}'.format(
        event_slack_string, call_to_action_string)
...
def _generate_event_slack_string(from_user, event, to_user, issue, reviewable_url):
    ...
    event_slack_string = _EVENT_SLACK_TEMPLATES[event].format(
...
def _get_comment_parts(comment_body):
    ...
    inline_comment_links = _REVIEWABLE_INLINE_COMMENT_LINK_REGEX.findall(
        inline_comments_block)
...
def _get_github_api_ressource(ressource_url):
    ...
    if not _GITHUB_PERSONAL_ACCESS_TOKEN:
        raise SetupException(
            'Need to define _GITHUB_PERSONAL_ACCESS_TOKEN env variable.')
