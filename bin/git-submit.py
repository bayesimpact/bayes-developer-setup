...
def _rebase_or_fail(
        onto: str, branch: str, *, interactive: bool = False, leave_dirty: bool = False) -> None:
...
def _handle_rebase(default: _Branch, branch: _Branch, force: bool = False) -> None:
    ...
    while default.initial != penultimate:
        ...
        if _is_ancestor(penultimate, default.initial):
            if force:
                _rebase_or_fail(default.tracked, branch.local, leave_dirty=leave_dirty)
            break
        logging.warning(
        ...
        if not _ask_yes_no('Rebase now?'):
            sys.exit(4)
        _rebase_or_fail(default.tracked, branch.local, interactive=True, leave_dirty=leave_dirty)
        penultimate = _Branch.get_sha1(f'{branch.local}^')
...
def main() -> None:
    ...
    should_abort = args.abort or args.rebase
    if should_abort and not args.user:
    ...
    remote_prefix = f'{default.remote}/{args.user}-' if should_abort else None
