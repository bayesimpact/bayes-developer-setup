...
class _RemoteGitPlatform:
    ...
    def __new__(cls, remote_url) -> '_RemoteGitPlatform':
        if gitlab_match := _GITLAB_URL_REGEX.match(remote_url):
            return _GitlabPlatform(gitlab_match)
...
class _GitlabPlatform(_RemoteGitPlatform):
    ...
    def _get_reviewers(self, reviewers: Optional[str]) -> List[str]:
...
def _request_review(refs: _References, reviewers: Optional[str]) -> None:
    ...
    _RemoteGitPlatform(remote_url).request_review(message, refs, reviewers)
